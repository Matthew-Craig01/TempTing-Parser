...
-spec equal(GME, GME) -> nil.
equal(A, B) ->
    gleeunit_ffi:should_equal(A, B).

-spec not_equal(GMF, GMF) -> nil.
not_equal(A, B) ->
    gleeunit_ffi:should_not_equal(A, B).

-spec be_ok({ok, GMG} | {error, any()}) -> GMG.
be_ok(A) ->
    gleeunit_ffi:should_be_ok(A).

-spec be_error({ok, any()} | {error, GML}) -> GML.
be_error(A) ->
    gleeunit_ffi:should_be_error(A).

-spec be_some(gleam@option:option(GMO)) -> GMO.
be_some(A) ->
    case A of
        {some, Value} ->
            Value;
